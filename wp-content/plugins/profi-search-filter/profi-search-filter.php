<?php
	...
	*/
	define( 'SF_CURRENT_VERSION', '1.72' );
	if( !session_id() )
		session_start();
	...
	*/
	...
	function get_all_postmetas_from_post_type( $post_type ){
		...
		$sql = $wpdb->prepare( "
			...
			WHERE $wpdb->posts.`post_type` IN (".implode(', ', array_fill(0, count($post_type), '%s')).") && $wpdb->postmeta.`post_id` = $wpdb->posts.`ID`
	...
	function get_postmeta_values( $meta_key ){
		global $wpdb;
		$sql = "select meta_value from " . $wpdb->prefix . "postmeta where meta_key = '" . mysql_real_escape_string( $meta_key ) . "' group by meta_value order by meta_value asc";
